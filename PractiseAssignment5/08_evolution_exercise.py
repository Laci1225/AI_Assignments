...
def selection_roulette(states: list[State],
    ...
    weights = [f(state) for state in states]
    list_of_new_states = choices(states, weights, k=len(states))
    return list_of_new_states
...
def recombine(state_a: State, state_b: State) -> tuple[State, State]:
    ...
    r_i, r_i2 = randint(0, 7), randint(0, 7)
    while r_i > r_i2:
        r_i2 = randint(0, 7)
    new_state_a = state_b[0:r_i] + state_a[r_i:r_i2] + state_b[r_i2:]
    new_state_b = state_a[0:r_i] + state_b[r_i:r_i2] + state_a[r_i2:]
...
def repair_states(state_a: State, state_b: State) -> tuple[State, State]:
    ...
    for i in range(state_a_.__len__()):
        if state_a_.count(state_a_[i]) > 1:
            for j in range(state_a_.__len__()):
                if state_b_[j] not in state_a_:
                    state_a_[i] = state_b_[j]
        if state_b_.count(state_b_[i]) > 1:
            for j in range(state_a_.__len__()):
                if state_a_[j] not in state_b_:
                    state_b_[i] = state_a_[j]
...
def mutate(state: State, chance: float) -> State:
    ...
    if random() < chance:
        ...
        while r_i2 == r_i:
            r_i2 = randint(0, 7)
        mutated_state[r_i], mutated_state[r_i2] = mutated_state[r_i2], mutated_state[r_i]
        return mutated_state
